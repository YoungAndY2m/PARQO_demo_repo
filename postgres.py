...
def get_real_latency(db_name, sql, hint=None, times=5, inject=False, output_plan=False, query_id=None, return_json=False, limit_time=15000, limit_worker=False, drop_buffer=True):
    # TODO inject or not is meaning less
    conn = psycopg2.connect(host="/tmp", dbname=db_name, user="lsh")
    conn.set_session(autocommit=True)
    cursor_ = conn.cursor()
    os.system("cp ./cardinality/new_single.txt /data/imdb/")
...
def get_plan_cost(cursor, sql, hint=None, debug=None, explain=None, plan=False):
    os.system("cp ./cardinality/join.txt /data/imdb/")
    os.system("cp ./cardinality/new_single.txt /data/imdb/")
    os.system("cp ./cardinality/pointers.txt /data/imdb")
    cursor.execute('DISCARD ALL;')
    cursor.execute('SET enable_material = off')
    # cursor.execute('SET top_n = 0')
    ...
    os.system("rm /data/imdb/join_est_record_job.txt")
    os.system("rm /data/imdb/single_tbl_est_record.txt")
