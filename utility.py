...
def cal_rel_selectivity(err, est_sel):
    if err > 0:
        tru_sel = math.exp(err) * est_sel
    else:
        tru_sel = est_sel / math.exp(-err)
    tru_sel = min(tru_sel, 1)
    tru_sel = max(tru_sel, 0)
    return tru_sel
...
def get_error_dict_from_txt():
    ...
    with open('/data/imdb/single_tbl_est_record.txt', 'r') as file:
        single = file.read()

    # Read the content of the second text file
    with open('/data/imdb/join_est_record_job.txt', 'r') as file:
        join = file.read()
